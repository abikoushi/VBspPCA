...
double up_theta(arma::mat & Z, 
  ...
  W = obs_prec*arma::trans(solve(NS*prec_w, num_w.t(), arma::solve_opts::likely_sympd));
  
  arma::mat WW = W.t() * W;
  prec_z = obs_prec*WW + prior;
  R = up_eta_z(num_z,y,rowi,coli, Z, W);
  Z = obs_prec*arma::trans(solve(NS*prec_z, num_z.t(), arma::solve_opts::likely_sympd));
  R += accu(WW%ZZ); //!!!
  return obs_prec*R;
}
...
List doVB_norm(const arma::vec & y,
  ...
  for (int i=0; i<iter; i++) {
    double lp = up_theta(Z, W, prec_z, prec_w, R, y, rowi, coli, obs_prec, prior_prec, 1.0);
...
List doVB_norm_s(const arma::vec & y,
  ...
  const double sumy2 = 0.5*accu(pow(y,2));
  for (int i=0; i<iter; i++) {
    double lp = up_theta(Z, W, prec_z, prec_w, R, y, rowi, coli, obs_prec, prior_prec, NS);
    //up_lambda(obs_prec, ahat, sumy2+R+b);
    logprob(i) = -0.5*obs_prec*(sumy2+R);
  }
  return List::create(Named("mean_z") = Z,
...
List doVB_norm_s(const arma::vec & y,
                 ...
                 const double & a,
                 const double & b){
  arma::uvec uid_r = unique(rowi);
  arma::uvec uid_c = unique(coli);
  arma::mat Z = arma::randn<arma::mat>(Nr, L);
  arma::mat W = arma::randn<arma::mat>(Nc, L);
  arma::mat prec_z;
  arma::mat prec_w;
  ...
  const double sumy2 = 0.5*accu(pow(y,2));
  for (int i=0; i<iter; i++) {
    double lp = up_theta_s(Z, W, prec_z, prec_w, R, y, rowi, coli, obs_prec, prior_prec);
    //up_lambda(obs_prec, ahat, sumy2+R+b);
    logprob(i) = -0.5*obs_prec*(sumy2+R);
  }
  return List::create(Named("mean_z") = Z,
